...
| 🏗️ **工程质量高** | ≈380 个 HTTP 端点（含别名）+ 4 条 WebSocket 状态通道；pytest **1152 用例**（GPU 集成显式开关）/ vitest **101 用例** / 文档对账闸 + 活后端 E2E 多层测试体系 |
...
### 🖥 系统与运维
- **存储清理中心**：磁盘占用盘点 / 一键清理 / 备份文件管理
- **硬件健康中心**：显存 / 温度 / 硬件自检实时可视
- **操作审计日志**：关键操作留痕可查；备份加密 + 断电自恢复；密钥保险箱（云端 Key 加密存放）
- **全局任务中心**：跨模块任务统一排队 / 取消 / 进度；Ctrl+K 全局搜索；生成物文件库（一处管理全部产出）
...
| **测试** | pytest（1152 用例，GPU 集成显式开关）+ vitest（101 用例）+ 活后端 E2E 流程编排 |
