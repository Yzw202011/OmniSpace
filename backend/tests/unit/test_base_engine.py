...
def test_fake_category_resolvable_via_registration_only() -> None:
    register_engine_module("fake_cat",
                           "backend.tests.unit.test_base_engine",
                           "_make_fake_engine",
...
def test_unregistered_and_broken_categories_degrade_to_none() -> None:
    ...
    register_engine_module("broken_cat", "backend.tests.no_such_module_xyz")
    try:
...
def test_all_first_batch_engines_implement_protocol() -> None:
    ...
    from backend.services.inference.video_engine import VideoEngine
    from backend.services.inference.voice_engine import VoiceEngine
...
def test_singleton_getter_parity_with_manager_resolution() -> None:
    ...
    from backend.services.inference.voice_engine import get_voice_engine
