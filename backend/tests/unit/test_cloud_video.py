...
import backend.services.video_queue as vq
...
from backend.services.inference import cloud_video_client as cvc
from backend.tests.unit.mock_cloud_video import MockCloudVideo
...
def test_video_wiring_sentinels() -> None:
    video_src = (_ROOT_DIR / "backend/api/manga/video.py").read_text(
        encoding="utf-8")
    ...
    queue_src = (_ROOT_DIR / "backend/services/video_queue.py").read_text(
        encoding="utf-8")
