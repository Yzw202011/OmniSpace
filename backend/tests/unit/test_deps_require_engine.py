...
from backend.api.deps import require_engine
from backend.middleware.error_handler import ApiError, error, ok
...
def hub(monkeypatch: pytest.MonkeyPatch) -> FakeHub:
    ...
    monkeypatch.setattr(
        __import__("backend.services.ws_hub", fromlist=["get_ws_hub"]),
