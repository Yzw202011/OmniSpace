...
KF_PY = Path(__file__).resolve().parents[2] / "api" / "manga" / "keyframe.py"
...
def test_serializer_and_migration() -> None:
    ...
    from backend.api.manga.common import _KF_COLS
    assert "source_mode" in _KF_COLS, "_KF_COLS 缺 source_mode"


def test_serializer_roundtrip() -> None:
    from backend.api.manga.keyframe import _kf_row_to_dict
