...
from backend.services import knowledge_checkup as kc
...
def test_run_checkup_reports_dirty_and_latency(
    ...
    monkeypatch.setattr("backend.services.knowledge_service.get_knowledge_service",
                        lambda: _FakeKbSvc())
    monkeypatch.setattr("backend.services.injection_service.get_injection_service",
                        lambda: _FakeInj())
