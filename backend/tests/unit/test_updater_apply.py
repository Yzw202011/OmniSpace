...
def fake_install(tmp_path: Path) -> Path:
    ...
    (root / "backend" / "main.py").write_text(OLD_MAIN, encoding="utf-8")
...
def _manifest() -> dict:
    return {
        ...
        "files": [
            {"path": "backend/main.py", "action": "replace",
...
def work_dir(tmp_path: Path) -> Path:
    ...
    (w / "backend" / "main.py").write_text(NEW_MAIN, encoding="utf-8")
...
def test_apply_replaces_deletes_and_backs_up(fake_install: Path,
    ...
    # 备份里是旧内容
    assert (backup_dir / "backend" / "main.py").read_text("utf-8") == OLD_MAIN
    assert (backup_dir / "old_extra.txt").read_text("utf-8") == "DELETE_ME"
    # 落位
    apply_payload(fake_install, work_dir, manifest)
    assert (fake_install / "backend" / "main.py").read_text("utf-8") == NEW_MAIN
...
def test_rollback_restores_old_files(fake_install: Path, work_dir: Path,
    ...
    assert errs == []
    assert restored >= 3
    assert (fake_install / "backend" / "main.py").read_text("utf-8") == OLD_MAIN
...
def test_boot_hook_recovers_interrupted_upgrade(
    ...
    (fake_install / "backend" / "main.py").write_text(NEW_MAIN, encoding="utf-8")
    backup_dir = fake_install / "updates" / "backup" / "20260911_120000"
    (backup_dir / "backend").mkdir(parents=True)
    (backup_dir / "backend" / "main.py").write_text(OLD_MAIN, encoding="utf-8")
    ...
    (fake_install / "updater" / "restore.py").write_text(
        (Path(__file__).resolve().parents[3] / "updater" / "restore.py")
    ...
    assert (fake_install / "backend" / "main.py").read_text(
        "utf-8") == OLD_MAIN
...
def test_boot_hook_noop_on_normal_state(
    ...
    (fake_install / "backend" / "main.py").write_text("LIVE", encoding="utf-8")
    monkeypatch.setattr(boot_mod, "BOOT_DIR", fake_install / "launcher")
