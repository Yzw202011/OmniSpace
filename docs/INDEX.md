...
| `底层架构升级方案-2026-09-08.md` | **底层架构升级（规划，未实施）** | 显存更省/性能更强/更智能的底座升级十批次（V1 vLLM 0.28 升级【前置=确认 Windows 轮子渠道】/ V2 ComfyUI 动态显存【默认开启已代码级实证】/ V3 显存分档+FP8 KV+长上下文 16K~32K / **V4 豆包云端【09-08 用户令延后挂起】**/ V5 基准尺子 / V6 MTP 投机解码【权重已在盘，含 #53912 雷区对照】/ V7 SageAttention / V8 torch 2.13+cu130【随 B 阶段二】/ V9 显存编排统一层 / V10 多卡收口【云端部分随 V4 延后】）；本地优先序 V2→V5→V1→V3→V6→V7→V9；同日两会话摸底合并成文，D1~D9 待拍板（D3 已定=延后）；拍板前不动码 |
...
## 七、批量补登记（2026-09-18 文档对账批，批3）

> 四轮审计「INDEX 结构断裂+39 孤儿」根修：本节把落款后游离的方案行归位，
> 并一次性补登记此前漏登的 38 份根目录文档。状态为登记时的诚实快照。

### 7.1 游离行归位（原误贴于文末落款之后）

| `B阶段二换画笔方案-2026-09-09.md` | **B 阶段二「换画笔」四段制方案（规划，待拍板）** | Qwen-Edit-2511 三图合成接入（GGUF 路线绕开 nunchaku——官方 Windows 轮最高 torch2.11 < 我们 2.13，实证矩阵在案）；三步走 B2-1 组件安装冒烟→B2-2 双路由基准实测→B2-3 拍板退役（进一件退一件净减 14G）；PyPI 同名 nunchaku 是占位包勿装；D-B2-1~3 待拍板 |
| `Z-Image接入与四视图重构方案-2026-09-15.md` | **Z-Image/四视图重构（已开工已入库 93ac411+7fb0957）** | Z1 引擎槽+Z2 zviews 分张管线+Z4 LoRA 训练全执行；**遗留拍板**：zviews 默认档（验收线 ≤45s 实测 69~84s 未达、监督会话标记提交未经拍板确认）与 cloud-first 烧配额语义（已提请知情未见回应） |
| `新技术加速方案对比-2026-09-15.md` | **五项加速方案对照（P-5 已开工，其余待拍板）** | DFlash 已接线默认关（A/B 脚本就绪）；P-1 步数档/P-2 H3 sage 实测/P-3 二采 1080p/P-4 T8 双时钟（后两项碰 H3 冻结令）待拍板 |
| `插件技能层接线方案-2026-09-17.md` | **技能插座层（✅ 批0~批3 已完工入库：981f83f/c902270/185fc11+4bd7e85/a33a3a8；批4 随 VideoMaking 移除失效转文档收口）** | 四创作功能（chat/novel/comic/manga）× 插件系统；manifest.skills 声明 + GET /plugins/skills 索引 + 每功能一个专用端点；安全全复用（三道闸/信任三档/沙箱）；出厂技能面为空（D3=A 插座先行） |
| `插件开发规范.md` | **插件开发规范 v1.3（2026-09-18 对表修订：manifest 全字段表/三套版本号口径/四类白名单/generic 边界/事件主题命名/route 非必填，响应 issue #4）** | 面向含第三方的开发者契约：两种形态三档信任/AST 机器闸清单/运行契约/§3.1 技能声明（三端点已上线）/包格式与 14 字段全表/开发者工作流/用户侧须知 |
 | `绘画方向拍板材料包-2026-09-17.md` | **绘画方向四拍板材料（四视图已销案；LoRA 接产线/纯真人重训/qwen-image/paint.default 待用户回字母）** | 现状对账+成本+风险+推荐；qwen-image 实测 RAM 17.2G<22G 建议挂起 |
 | `全栈审计修复计划-2026-09-18.md` | **四轮全栈扫透的修复总账（批0~批5 排期，全部待拍板）** | 批0 止血四件（出包修复/锁重导/WS-SSE 定夺/klein-9b 病历）→批1 清残大扫除→批2 防御补齐→批3 结构工程（训练抽基座/W3-C/数字码/时区）→批4 观测契约→批5 长期治理；真源=记忆 audit-2026-09-18-fullstack-sweep |

### 7.2 漏登文档批量登记（38 份）

| 文档 | 类型 | 状态一句话 |
|------|------|-----------|
| `ADR-004-desktop-shell-restart.md` | 决策记录 | 桌面壳关页重启决策（现行） |
| `AI能力升级计划-2026-09-04.md` | 方案 | 阶段一换脑子已执行；二三阶段待拍板 |
| `B5灰度实测验收计划-2026-09-14.md` | 记录 | B5 观察期验收（已收官，legacy 内联已删） |
| `H3加速三项实施方案-2026-09-16.md` | 方案 | sage +18% 已收口默认开；步数档 balanced=8 已拍板执行；T8 双时钟判伪默认关 |
| `UX合理性补全方案-2026-09-07.md` | 方案 | 历史快照（部分已随后续迭代吸收） |
| `backlog-前端待接线端点清单-2026-09-16.md` | 记录 | 大部分已随 09-17 七簇接线入库 |
| `uat-2026-09-10-执行记录.md` | 记录 | UAT 战报存档 |
| `云端API接入方案-2026-09-06.md` | 方案 | 批1~3 已执行+真实 Key 实弹通过 |
| `产品介绍-客户版-2026-09-03.md` | 现行材料 | 对外宣传口径 |
| `体积优化方案-2026-09-10.md` | 方案 | 部分执行（A4~A6 收官） |
| `全量技术改造与升级方案-2026-09-12.md` | 方案 | v3 前身（已收官） |
| `全量技术改造与升级方案-v3-对照重构-2026-09-13.md` | 方案 | v3 已全线收官（六笔入库） |
| `全量技术评估总汇总与修复总方案-v4-2026-09-13.md` | 方案 | 现行总修复方案（真源之一） |
| `升级机制方案-2026-09-08.md` | 方案 | 已收官（外部 updater+回滚+实弹） |
| `多角色一致性路由方案-2026-09-10.md` | 方案 | 已收官（Step A/B 双 PuLID 转正） |
| `技术调研-AI小说与AI漫画及多显卡专业卡适配-2026-09-05.md` | 调研 | 调研记录（D1~D3 已拍板执行） |
| `插件用户导入方案-2026-09-16.md` | 方案 | 已执行 |
| `新模块与多显卡实施计划-2026-09-05.md` | 方案 | 批1~3 入库；实机双卡验证缺口在案 |
| `方案书-src扁平化与内核引入-补交-2026-09-16.md` | 方案 | 已执行（src 扁平化落地） |
| `显存深度优化方案书-2026-09-14.md` | 方案 | 记录（部分随深度实测轮吸收） |
| `显存调度机制方案-2026-09-10.md` | 方案 | 已收官（账本/准入/单一编排者） |
| `显存账本口径统一方案-2026-09-16.md` | 方案 | 成文待拍板 |
| `未完成清单.md` | 现行 | 跨模块待办真源 |
| `架构升级计划-2026-09-05.md` | 方案 | 现行总路线图（A1/A2/A5 入库，B 阶段一大部分） |
| `治愈系主题方案-2026-09-11.md` | 方案 | D1~D4 已拍；预览件四态过验，等挑 A/B+D5/D6 |
| `测试报告-显存调度-2026-09-10-第一轮.md` | 记录 | 测试存档 |
| `测试方案-显存调度-2026-09-10.md` | 记录 | 测试存档 |
| `深度实测轮计划-2026-09-14.md` | 方案 | S1~S7 已执行（绘画五轮 A/B 在案） |
| `漫画模块升级方案-参考AIMangaStudio-2026-09-08.md` | 方案 | 已收官（C2~C4+资产库） |
| `漫画模块立项方案-2026-09-07.md` | 方案 | 已收官（M0+M1） |
| `知识学习模块升级方案-2026-09-05.md` | 方案 | 已收官（批0~5+二轮加固） |
| `编程语言规范例外清单.md` | 现行 | 规范例外真源（合规配套） |
| `编程语言规范合规计划-2026-09-04.md` | 方案 | 已收官（批0~4） |
| `自愈与横切内建方案-2026-09-10.md` | 方案 | 已收官（批1~4 单日四批） |
| `自换大模型支持方案-2026-09-10.md` | 方案 | H3 默认主力已拍板；SD15/SDXL/视频档待 |
| `路由幻影候选治理方案-2026-09-16.md` | 方案 | 成文待拍板 |
