...
**RTM 对应**：F-06（分镜四端点 ✅）、F-07（导演台 /export ❌）、F-08（解说漫剧 ✅）、A-04（Three.js ✅）、M-03/M-13（视频模型 ✅）。出图规格铁律：资产图/分镜图统一 2560×1440（SDXL 1280×720 + LANCZOS 2x 上采样，2026-08-14 用户裁定，代码 manga.py IMG_TARGET_W/H）。**四视图（2026-08-20 用户裁定，回归铁律规格 + 同日 v2 质量闭环）**：角色四视图走竞品 one-pass 技术路线——FLUX.2 Klein（Qwen3 中文文本编码器）v2 中文长提示词（人设前置 + 全正向语义，无负向禁令——4B 蒸馏模型对否定语义遵循差）全文直入，1 次推理单图 2560×1440（16:9，对齐根目录 参考图.png 版式：1 行 4 列等宽竖格，每格 640×1440 全身竖构图）内出四视图；**生成后 VL 质检闭环**（本地 qwen3-vl-4b）：逐格判定视角组合（正面/侧面/背面/特写）与剪影/残缺异常，错位格（≤2）走 FLUX.2 单格 inpaint 局部修复后复检，未收敛换 seed 整图重 roll（上限 3 轮，meta.layout_verified 如实记录）；SAM 人物分割背景漂白保底纯白底（人物完整优先）；PIL 叠加中文标注（左上角角色名 + 底部视图标签，白底圆角衬底），等分裁切 portrait_views/ + master.png 干净底图；单视图重生走 master.png 整格 inpaint（竞品不支持，本实现超越）；FLUX.2 不可用回退 SDXL 逐视图（canvas 1×4 横排 contain 拼图）并诚实标 degraded。代码 comic_gen.py `_generate_turnaround_onepass`/`_verify_view_layout`/`_repair_view_cell`。
