...
**用旧版程序打开了新版程序产生的数据库**（版本回退场景）：迁移机制用 `PRAGMA user_version` 携带版本号（当前 `SCHEMA_VERSION=13`——批0-4 2026-09-18 实测 database.py:484 与生产库 user_version 均为 13；v3=敏感字段加密存量迁移，v4~v13 为后续迁移组），库版本高于代码版本说明库里已有代码不认识的新列/新表/数据格式，强行打开会造成静默数据损坏（如旧代码读加密库会把密文当明文返回），因此启动时直接 RuntimeError 拒绝——这是守护，不是 bug。
...
输出 `7` = 库与代码同级（v7），冲突另有原因；`>7` = 确认版本冲突。
