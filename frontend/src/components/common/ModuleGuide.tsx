...
const GUIDE_CONTENT: Record<string, ModuleGuideContent> = {
  ...
  // ── 2026-09-19 批1（P1 全页面引导补齐）：以下六页接入 ──
  manga: {
    key: 'manga',
    title: '漫剧创作 · 新手引导',
    items: [
      '新建作品选类型（普通/解说），先进「剧本录入」——直接粘贴、AI 切分或 DSL 导入',
      '顶部工序条按「角色推理 → 分镜生词 → 分镜生图 → 生成视频」推进，点击直达',
      '生词/生图前先把角色等资产绑定到分镜行（右栏资产库点选），一致性靠它',
      '右上「生成记录」看每次生图/生视频的成功失败与耗时，失败可重试',
    ],
  },
  learning: {
    key: 'learning',
    title: '知识学习 · 新手引导',
    items: [
      '「让 AI 自己决定学什么」一键开learn，也可添加固定主题定向学习',
      '学习中点「查看浏览器」实时围观，随时可「用户接管」亲自操作',
      '学到的知识进「知识库」：支持搜索/图谱、批量删除、图片知识',
      '「学习设置」控制时长/页数/流量等限额，行为学习在后台记录偏好',
    ],
  },
  training: {
    key: 'training',
    title: '训练中心 · 新手引导',
    items: [
      '三个页签：知识训练（知识库→LoRA）、风格训练（视频素材→画风）、人物训练（角色图→身份LoRA）',
      '顶部统一任务队列显示所有训练的进度，训练与生成互斥属正常保护',
      '人物 LoRA 训练完成后，该角色在漫剧生图会自动挂载（一致性更稳）',
      '风格训练产物在创作页画风列表立即可选；版本支持对比与回滚',
    ],
  },
  models: {
    key: 'models',
    title: '模型管理 · 新手引导',
    items: [
      '顶部「就绪度」横幅告诉你缺什么模型，照指引补齐即可',
      '模型按显存档位分组；「选择」给对应模块设默认，「加载」后才能用',
      '重量级功能互斥：对话/绘画/视频/训练互相让位是正常调度，不是故障',
      '「性能基准」可跑分对比；导入模型支持外部路径直接登记',
    ],
  },
  settings: {
    key: 'settings',
    title: '设置 · 新手引导',
    items: [
      '六个标签：通用/外观/AI服务/数据/插件/维护',
      '云端 API 在「AI服务」签添加连接并绑定工位（对话/图片/视频可走云端）',
      '备份与恢复在「数据」签（恢复后需重启生效）；「维护」签有一键体检与升级',
      '「界面打开方式/性能模式」等设置按提示即时或下次启动生效',
    ],
  },
  logs: {
    key: 'logs',
    title: '日志 · 新手引导',
    items: [
      '四个视图：事件时间线 / 执行流程 / 技术日志 / 资源曲线',
      '排障先看「错误聚合」卡，再按关键词过滤技术日志定位细节',
      '「导出诊断包」一键打包（自动脱敏），发给维护者即可',
      '事件默认保留 30 天，可手动清理过期日志',
    ],
  },
};
