...
/** 六大模块简介 */
const MODULES: Array<{ icon: string; name: string; desc: string }> = [
  { icon: '💬', name: 'AI对话', desc: '与本地大模型多轮对话，支持知识库增强问答，回答引用已学习的知识。' },
  { icon: '🎨', name: 'AI绘画', desc: '文生图 / 图生图，支持批量出图（1~4）与高清修复；ControlNet 与绘画 LoRA 叠加尚未开放，页面已如实标注。' },
  { icon: '🎬', name: '漫剧创作', desc: '分镜表驱动的漫剧流水线：剧本拆解、机位管理、截图资产与语音绑定。' },
  { icon: '📚', name: '知识学习', desc: 'AI 自主联网学习：主题管理、实时浏览查看、行为学习与知识库管理。' },
  { icon: '🎯', name: '视频风格', desc: '上传视频/图片素材训练专属风格 LoRA，支持版本管理、回滚与风格化预览帧。' },
  { icon: '📦', name: '模型管理', desc: '本地模型的导入、加载、卸载与显存占用监控，协同调度一目了然（离线版本不提供在线下载）。' },
];
    ...
    <div className="page">
      ...
      <div className="flex flex-col gap-5 mt-5">
        ...
        <section className="card hoverable" aria-label="功能模块">
          <h3 className="card-title">🧭 功能模块</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
            {MODULES.map((m) => (
              ...
              >
                ...
                <div className="text-secondary mt-2" style={{ fontSize: 'var(--font-size-xs)', lineHeight: 'var(--line-height-relaxed)' }}>
                  {m.desc}
                </div>
              </div>
            ))}
        ...
        <section className="card hoverable" aria-label="快速上手">
          <h3 className="card-title">🚀 快速上手（5 步）</h3>
          <ol className="flex flex-col gap-2" style={{ paddingLeft: 'var(--space-6)' }}>
        ...
        <section className="card hoverable" aria-label="快捷键">
          <h3 className="card-title">⌨️ 快捷键</h3>
        ...
        <section className="card hoverable" aria-label="常见问题">
          <h3 className="card-title">💡 常见问题</h3>
