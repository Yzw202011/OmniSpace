"""知识训练 API 路由（/learn = 训练任务与数据集；≠ /learning 学习会话域）。

域边界（防混淆，问题总账 #36）：本模块 `/learn` 负责**知识训练**（QLoRA
任务/数据集/版本回滚）；浏览器代理学习会话/主题/调度在 `/learning`
（learning.py，前端 12 组件的大域）。两者非同域，路由与前端组件请勿混用。
