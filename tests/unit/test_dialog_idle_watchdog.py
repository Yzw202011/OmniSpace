...
ENGINE_PY = (Path(__file__).resolve().parents[2] / "src" / "services" / "inference" / "dialog_engine.py")
SRC = ENGINE_PY.read_text(encoding="utf-8")
CONFIG_PY = (Path(__file__).resolve().parents[2] / "src" / "config.py").read_text(
    encoding="utf-8")
...
def test_send_preflight_queues_instead_of_dropping() -> None:
    ...
    api_src = (Path(__file__).resolve().parents[2] / "src" / "api" / "dialog.py").read_text(encoding="utf-8")
