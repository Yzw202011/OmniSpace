...
from src.services.inference import gpu_budget
from src.services.inference.gpu_budget import (
    EXTERNAL_ANOMALY_GB,
    BusyRegistry,
    GpuBudget,
    Verdict,
...
_ROOT = Path(__file__).resolve().parents[2]  # 仓库根（unit/ → tests/ → src/ → 根）
...
class TestCpuAssistTwoState:
    ...
    def test_locked_generation_period_skips_offload(
        ...
        from src.services.scheduler.decision import DecisionEngine
        from src.services.scheduler.dispatcher import TaskDispatcher
        ...
        events: list[str] = []
        monkeypatch.setattr(
            "src.services.event_log.log_event",
...
class TestCloudBusyAdmit:
    ...
    def test_admit_survives_registry_failure(
        ...
        monkeypatch.setattr(
            "src.services.inference.gpu_budget.get_busy_registry", _boom)
...
class TestModelsVramEndpoint:
    ...
    def test_endpoint_degrades_when_budget_blind(
        ...
        monkeypatch.setattr(
            "src.services.inference.gpu_budget.get_gpu_budget", _boom)
...
class TestIdleConsumerWiring:
    ...
    def test_all_idle_checks_registry(self) -> None:
        src = (_ROOT / "src" / "services" / "scheduler" / "decision.py"
               ).read_text(encoding="utf-8")
