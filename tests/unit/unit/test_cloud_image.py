...
import src.services.image_queue as iq
...
from src.services.inference import cloud_image_client as cic
from tests.unit.unit.mock_cloud_image import MockCloudImage
...
def test_image_wiring_sentinels() -> None:
    draw_src = (_ROOT_DIR / "src/api/draw.py").read_text(encoding="utf-8")
    ...
    kf_src = (_ROOT_DIR / "src/api/manga/keyframe.py").read_text(
        encoding="utf-8")
    ...
    asset_src = (_ROOT_DIR / "src/api/manga/comic_asset.py").read_text(
        encoding="utf-8")
    assert 'get_image_endpoint("asset.image")' in asset_src

    common_src = (_ROOT_DIR / "src/api/manga/common.py").read_text(
        encoding="utf-8")
