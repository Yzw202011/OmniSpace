...
@pytest.fixture()
def mock_vllm():
    from tests.unit.unit.mock_remote_vllm import FakeRemoteVLLM
...
def test_engine_first_cloud_load_with_dialog_local(
    ...
    from src.services.inference.backends.remote_backend import RemoteDialogBackend
...
def test_record_cloud_call_never_raises(
    ...
    import src.services.event_log as el
...
def test_probe_cloud_api_states(mem_kv, monkeypatch: pytest.MonkeyPatch) -> None:
    from src.api.system import _probe_cloud_api
...
def test_dialog_ws_whitelist_passes_cloud_model() -> None:
    ...
    分支逻辑单验。
    """
    src = (_ROOT_DIR / "src/api/dialog.py").read_text(encoding="utf-8")
