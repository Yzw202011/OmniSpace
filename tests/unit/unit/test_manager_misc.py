...
import src.services.scheduler.monitor as _mon
from src.data.models import ModelCategory
from src.services.model_manager.classifier import ModelClassifier
from src.services.model_manager.selector import ModelSelector
from src.services.scheduler.monitor import HardwareMonitor
...
def test_selector_downloaded_scan_failure_tolerated(monkeypatch) -> None:
    import src.services.model_manager as _mm
    def _boom():
...
def test_selector_dispatch_tables(monkeypatch) -> None:
    from src.data.models import DIALOG_ROUTING_TABLE, PAINT_ROUTING_TABLE
    s = ModelSelector()
    seen: list = []
...
def test_selector_skips_not_downloaded_higher_tier(monkeypatch) -> None:
    from src.data.models import DIALOG_ROUTING_TABLE
    s = ModelSelector()
