"""A/B 第四轮续·纯提示词直出（无 PuLID/LoRA/参考图）。
  纯B = comfy 9b 纯 txt2img
  纯A = legacy 4b 纯 txt2img
同描述词/同 seed/同尺寸，与 ab4 三张构成完整对照。
"""
import sys, time
...
r["images"][0].save(OUT / "ab4_gc_pure9b.png", "PNG")
...
r = pe.generate({"prompt": PROMPT, "negative": "", "seed": SEED,
                 "steps": 20, "cfg": 7.5, "sampler": "euler_a",
                 "width": 512, "height": 768})
r["images"][0].save(OUT / "ab4_gc_pure4b.png", "PNG")
print(f"[纯A legacy4b 纯文字] wall={time.perf_counter()-t0:.1f}s", flush=True)
print("AB4-PURE-DONE")
